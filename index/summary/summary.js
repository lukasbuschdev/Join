...
export function renderBoardTitleSelection() {
  ...
  $("#board-title-selection .options").innerHTML = Object.values(
    ...
    (template, board) =>
      `${template}${
        activeBoardId != board.id ? boardTitleSelectionTemplate(board) : ""
      }`,
    ``
  );
}
...
export async function createNewBoard() {
  ...
  if (!titleIsValid)
    return throwErrors({ identifier: "title", bool: titleIsValid });
...
export const filterDrp = debounce(() => {
  ...
  const sortedContacts = Object.values(STORAGE.currentUserContacts).sort(
...
export function addCollaborator(id) {
  ...
  if (NEW_BOARD_COLLABORATORS.includes(`${id}`))
    NEW_BOARD_COLLABORATORS.remove(`${id}`);
