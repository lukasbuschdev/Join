...
export function renderContacts() {
  ...
  $("#selected-contact-container").classList.toggle(
    "d-none",
    !contactsData.length
  );

  const initialLetters = new Set(
    contactsData.map((singleContact) => singleContact.name[0])
  );
...
export function renderSelectedContact(selectedContact) {
  $("#selected-contact-container").innerHTML =
    selectedContactTemplate(selectedContact);
}

export function selectedContactTemplate({
  id,
  img,
  name,
  email,
...
export async function addContact() {
  ...
  $("#add-contact-modal").closeModal();
  notification(
    `friendship-request, {name: '${STORAGE.data.users[selectedUserId].name}'}`
  );
}
