...
class Language {
  ...
  #get(lang) {
    if (!this.#data)
      throw Error(`Lang not initialized! await LANG.init() to fix.`);
          ...
          const data = (
            await Promise.all(
              ["init", "index"].map(async (dir) => {
                return (
                  await fetch(`${this.assetDir}/${dir}/${lang}.json`)
                ).json();
              })
            )
          ).reduce(
            (combinedData, langData) => ({ ...combinedData, ...langData }),
            {}
          );
          return [lang, data];
        })
  ...
  render(element = document) {
    element
      .querySelectorAll(
        "[data-lang], [data-lang-placeholder], [data-lang-empty]"
      )
  ...
  #renderDocumentTitle() {
    const titleText =
      this.currentLangData[`title-${currentDirectory().replaceAll("_", "-")}`];
  ...
  #renderSingleElement = (element) => {
    ...
    if (lang && lang.includes(", {"))
      return this.#renderWithOptions(element, lang);
    const langText = this.currentLangData[langKey];
    if (langEmpty)
      return (element.dataset.type = this.currentLangData[langKey]);
...
export const getEmailLanguage = async (type) => {
  ...
  type = type.convert();
  const data = await (
    await fetch(`/Join/assets/languages/mail/${LANG.currentLang}.json`)
  ).json();
  return Object.entries(data).reduce((total, [key, value]) => {
    if (key.includes(type))
      return { ...total, [key.replace(`${type}-`, "")]: value };
    else return total;
  }, {});
};